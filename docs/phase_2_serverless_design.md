...
- **Backend Optimization:** The `/api/imports/:id/status` query performs a non-blocking SELECT of _only_ the state and counters (`validRows`, `invalidRows`, `committedRows`, `totalRows`), completing in <5ms.
...
export function useImportStatus(jobId: string | null) {
  const { data, error } = useSWR(jobId ? `/api/imports/${jobId}/status` : null, fetcher, {
