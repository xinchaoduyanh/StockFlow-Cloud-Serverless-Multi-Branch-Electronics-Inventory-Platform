  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [aws_lambda_function.fn["reconciliation"].arn]
      },
      {
        Sid      = "SendReportJob"
        Effect   = "Allow"
      ...
      },
      {
        ...
        Resource = aws_sqs_queue.report_jobs_dlq.arn
      },
      {
        Sid      = "StartImportRecovery"
        Effect   = "Allow"
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "REPORT_DISPATCH_MODE", value = "sqs" },
      { name = "REPORT_QUEUE_URL", value = aws_sqs_queue.report_jobs.url },
      { name = "REPORT_QUEUE_DLQ_URL", value = aws_sqs_queue.report_jobs_dlq.url },
      { name = "REPORT_MAX_RECEIVE_COUNT", value = tostring(var.report_max_receive_count) },
      { name = "REPORT_PROCESSING_LEASE_SECONDS", value = tostring(var.report_queue_visibility_timeout_seconds) },
      { name = "IMPORT_STATE_MACHINE_ARN", value = aws_sfn_state_machine.ingestion.arn },
      { name = "IMPORT_RECOVERY_QUEUE_URL", value = aws_sqs_queue.import_recovery.url },
      { name = "IMPORT_RECOVERY_DLQ_URL", value = aws_sqs_queue.import_recovery_dlq.url },
      { name = "IMPORT_APPROVAL_TIMEOUT_SECONDS", value = tostring(var.import_approval_timeout_seconds) },
      { name = "IMPORT_MAX_REPLAY_COUNT", value = "3" },
      { name = "NOTIFICATION_SNS_TOPIC_ARN", value = aws_sns_topic.notifications.arn },
      { name = "SNS_CALLBACK_ALLOW_LOCAL", value = "false" },
