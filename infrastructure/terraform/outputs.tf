...
# 3 ARN này Phase 6 inject vào env API (ECS) để API gọi trực tiếp
output "report_exporter_lambda_arn" {
    value = aws_lambda_function.fn["report-exporter"].arn
}

output "dlq_replay_lambda_arn" {
    value = aws_lambda_function.fn["dlq-replay"].arn
}
